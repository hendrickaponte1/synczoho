 ...
 * @updated 2026-01-20 - Added geolocation support for country/region variables
 */
(function() {
  'use strict';
  ...
  var CONFIG = {
    ...
    CACHE_TTL: 60 * 1000, // 1 minuto en ms
    GEO_CACHE_TTL: 30 * 60 * 1000, // 30 minutos para geolocalización
    WIDGET_ID: 'tiendasync-delivery-widget',
    DEBUG: false,
    NO_CACHE: false // Se activa con ?nocache=1 en URL
  };

  // Detectar modo sin caché para testing
  ...
  // Timer para actualizar segundos
  ...
  var geoLocation = {
    ...
    regionName: null,
    city: null,
    source: 'pending' // 'ip', 'selector', 'fallback'
  };
  ...
  function getCountryFlag(countryCode) {
    ...
    // Generar emoji de bandera usando código de país (funciona para la mayoría)
    if (code.length === 2) {
      try {
  ...
  // ============================================
  // MONITOREO DEL SELECTOR DE PAÍS
  // ============================================
  function setupCountrySelector() {
    // Selectores comunes para el selector de país en Tiendanube
    var selectors = [
    ...
    selectorElement.addEventListener('change', function(e) {
      ...
      geoLocation = {
        ...
        regionName: geoLocation.regionName, // Mantener región si existe
    ...
    // También observar el input de código postal
  ...
  function setupPostalCodeListener() {
    ...
    postalInputs.forEach(function(input) {
      // Debounce para no hacer demasiadas actualizaciones
      ...
      input.addEventListener('input', function(e) {
        ...
        debounceTimer = setTimeout(function() {
          ...
          if (postalCode.length >= 4) {
            ...
            // En una implementación más avanzada, aquí se podría hacer
            // una consulta para obtener la región basada en el código postal
            // Por ahora solo actualizamos si hay un cambio
            if (currentConfig) {
              renderWidget(currentConfig);
            }
          }
  ...
  // Observador de mutaciones para detectar cuando se carga el selector dinámicamente
    ...
    var observer = new MutationObserver(function(mutations) {
      mutations.forEach(function(mutation) {
        if (mutation.addedNodes.length > 0) {
          // Buscar si se agregó un selector de país
  ...
  function getCachedConfig(storeId, productId) {
    // Saltear caché si está desactivado
    if (CONFIG.NO_CACHE) {
  ...
  function setCachedConfig(storeId, productId, config) {
    // No guardar caché si está desactivado
    if (CONFIG.NO_CACHE) {
      return;
    }
  ...
  // Función para limpiar caché manualmente (accesible desde consola)
  ...
  function fetchConfig(storeId, productId, callback) {
    ...
    .then(function(config) {
      ...
      // Diagnóstico: guardar template recibido
  ...
  function formatDate(date, todayLabel, tomorrowLabel) {
    var today = new Date();
    ...
    if (compareDate.getTime() === today.getTime()) {
      return todayLabel || 'hoy';
    }
    if (compareDate.getTime() === tomorrow.getTime()) {
      return tomorrowLabel || 'mañana';
    }
  ...
  // Formato corto: "martes 21" (sin mes, excepto si es otro mes)
  function formatDateShort(date, todayLabel, tomorrowLabel) {
    ...
    // Si es del mismo mes que hoy, formato corto sin mes
    if (date.getMonth() === today.getMonth() && date.getFullYear() === today.getFullYear()) {
      return days[date.getDay()] + ' ' + date.getDate();
    }
    
    // Si es otro mes, incluir el mes
    var months = ['enero', 'febrero', 'marzo', 'abril', 'mayo', 'junio', 
                  'julio', 'agosto', 'septiembre', 'octubre', 'noviembre', 'diciembre'];
    return days[date.getDay()] + ' ' + date.getDate() + ' de ' + months[date.getMonth()];
  }
  ...
  function calculateDeliveryDates(config) {
    ...
    // Usar configuración del producto si existe, sino la global
    ...
    // Aplicar modos de cálculo
    var prepCalcMode = delivery.preparation_calc_mode || 'min-max';
    var shipCalcMode = delivery.shipping_calc_mode || 'min-max';
    
    // Ajustar según modo de preparación
    if (prepCalcMode === 'none') {
      prepMinDays = 0;
      prepMaxDays = 0;
    } else if (prepCalcMode === 'min-only') {
      prepMaxDays = prepMinDays;
    }
    
    // Ajustar según modo de envío
    if (shipCalcMode === 'none') {
      shipMinDays = 0;
      shipMaxDays = 0;
    } else if (shipCalcMode === 'min-only') {
      shipMaxDays = shipMinDays;
    }
    ...
    // Si pasó la hora de corte, empezar desde mañana
    if (cutoffInfo.isPast) {
      startDate.setDate(startDate.getDate() + 1);
    }
    
    // Si hoy no es día laboral, avanzar al próximo
    while (!isWorkingDay(startDate, workingDays)) {
      startDate.setDate(startDate.getDate() + 1);
    }
    ...
    // Calcular fechas de preparación
    ...
    // Determinar si es hoy o mañana para variable especial
    var today = new Date();
    today.setHours(0, 0, 0, 0);
    ...
    if (minDateCompare.getTime() === today.getTime()) {
      ...
      var tomorrow = new Date(today);
      tomorrow.setDate(tomorrow.getDate() + 1);
      if (minDateCompare.getTime() === tomorrow.getTime()) {
        hoyOMañana = 'mañana';
      }
    }
    
    // Formatear días laborales como texto
  ...
  // Formatear días laborales en texto compacto (ej: "Lun-Vie" o "Lun, Mar, Jue")
  ...
  // ============================================
  // REEMPLAZO DE VARIABLES (ROBUSTO)
  // ============================================
  function replaceVariables(template, dates) {
    // Paso 1: Normalizar template (entidades HTML y formatos alternativos)
    ...
    // Paso 2: Convertir corchetes a llaves para variables conocidas
    var knownVariables = [
      ...
      'min_date', 'max_date', 'cutoff_time', 'hours_remaining', 
      'minutes_remaining', 'country_flag', 'country_name_advanced'
    ];
    ...
    // Paso 3: Diccionario de reemplazos
    var replacements = {
      // Variables en español (principales)
      ...
      // Variables de país/región con geolocalización real
      ...
      // Variables legacy en inglés (compatibilidad)
      '{min_date}': '<strong>' + dates.minFormatted + '</strong>',
      '{max_date}': '<strong>' + dates.maxFormatted + '</strong>',
      '{cutoff_time}': dates.cutoffTime,
      '{hours_remaining}': String(dates.cutoffInfo.hoursRemaining),
      '{minutes_remaining}': String(dates.cutoffInfo.minutesRemaining),
      '{country_flag}': countryFlag,
      '{country_name_advanced}': '<span class="tiendasync-country-name tiendasync-geo-dynamic">' + countryName + '</span>'
    };

    // Paso 4: Aplicar reemplazos
    ...
    // Paso 5: Diagnóstico - detectar tokens sin reemplazar
    if (CONFIG.DEBUG) {
  ...
  function createWidgetStyles(config) {
    ...
    return '\
      .tiendasync-widget {\
        margin: ' + app.margin_top + 'px ' + app.margin_right + 'px ' + app.margin_bottom + 'px ' + app.margin_left + 'px;\
        font-family: inherit;\
        box-sizing: border-box;\
      }\
      .tiendasync-message {\
        padding: 12px 16px;\
        background-color: ' + app.message_background_color + ';\
        color: ' + app.message_text_color + ';\
        border: ' + app.message_border_width + 'px ' + app.message_border_style + ' ' + app.message_border_color + ';\
        border-radius: ' + app.message_border_radius + 'px;\
        font-size: 14px;\
        line-height: 1.5;\
        display: flex;\
        align-items: center;\
      ...
      .tiendasync-message svg {\
        flex-shrink: 0;\
        width: 20px;\
        height: 20px;\
      }\
      .tiendasync-countdown {\
      ...
      .tiendasync-progress {\
        ...
        padding: 16px;\
        background: #fff;\
        border: 1px solid ' + app.progress_line_color + ';\
        border-radius: 8px;\
      }\
      .tiendasync-progress-bar {\
        display: flex;\
        align-items: center;\
        justify-content: space-between;\
        position: relative;\
      }\
      .tiendasync-progress-bar::before {\
        content: "";\
        position: absolute;\
        top: 50%;\
        left: 32px;\
        right: 32px;\
        height: 2px;\
        background: ' + app.progress_line_color + ';\
        transform: translateY(-50%);\
        z-index: 0;\
      }\
      .tiendasync-progress-step {\
        display: flex;\
      ...
      .tiendasync-progress-icon {\
        ...
        height: 40px;\
        border-radius: 50%;\
        background: ' + app.progress_icon_bg_color + ';\
        display: flex;\
        align-items: center;\
        justify-content: center;\
        margin-bottom: 8px;\
      }\
      .tiendasync-progress-icon svg {\
        width: 20px;\
        height: 20px;\
        color: ' + app.progress_icon_color + ';\
      }\
      .tiendasync-progress-icon.active {\
        background: ' + app.progress_icon_color + ';\
      }\
      .tiendasync-progress-icon.active svg {\
        color: #fff;\
      }\
      .tiendasync-progress-title {\
        font-size: 12px;\
        font-weight: 500;\
        color: ' + app.progress_title_color + ';\
        margin-bottom: 2px;\
      }\
      .tiendasync-progress-date {\
        font-size: 11px;\
        color: ' + app.progress_date_color + ';\
      }\
      .tiendasync-geo-dynamic {\
        transition: opacity 0.2s ease;\
      }\
      .tiendasync-geo-updating {\
        opacity: 0.6;\
      }\
    ';
  }
  ...
  function createProgressBarHTML(config, dates) {
    ...
    var steps = [
      ...
      },
      {
        icon: 'package',
        title: 'Preparado',
        date: dates.prepMinFormatted,
        active: false
      },
      {
        icon: 'map-pin',
        title: 'Entregado',
        date: dates.minFormatted,
        active: false
      }
    ];
  ...
  function findWidgetContainer() {
    // Buscar contenedor manual primero
    var manualContainer = document.getElementById('tiendasync-container') || 
                          document.getElementById('delivery-widget');
    if (manualContainer) {
      log('Using manual container');
      return { element: manualContainer, position: 'inside' };
    }
    
    // Selectores comunes de botón de compra en Tiendanube
    var selectors = [
      '.js-addtocart',
      '.js-add-to-cart', 
  ...
  function startCountdown(config) {
    ...
    // Solo iniciar countdown si hay {segundos_restantes} en el template
    if (config.texts.message_template.indexOf('segundos_restantes') === -1) {
      return;
    }
    ...
    countdownInterval = setInterval(function() {
      ...
      if (messageElement) {
        messageElement.innerHTML = replaceVariables(config.texts.message_template, dates);
      }
    }, 1000);
  }
  ...
  function renderWidget(config) {
    ...
    // Limpiar intervalo de countdown
    if (countdownInterval) {
      clearInterval(countdownInterval);
    ...
    if (!container) {
      error('Could not find container for widget');
      return;
    }
    ...
    if (CONFIG.DEBUG) {
      ...
      console.log('  Fechas calculadas:', {
        minFormatted: dates.minFormatted,
        maxFormatted: dates.maxFormatted,
        prepMinFormatted: dates.prepMinFormatted,
        prepMaxFormatted: dates.prepMaxFormatted,
        workingDaysText: dates.workingDaysText
      });
    ...
    // Iniciar countdown si es necesario
  ...
  function init() {
    ...
    var lsData = getLSData();
    if (!lsData || !lsData.storeId) {
      log('Not on a product page or LS not available');
      return;
    }
    
    log('Store ID:', lsData.storeId, 'Product ID:', lsData.productId);
    
    // Iniciar geolocalización en paralelo
    fetchGeolocation(function(err, geo) {
      if (err) {
        log('Geolocation error (using fallback):', err);
      } else {
        log('Geolocation obtained:', geo);
      }
      
      // Re-renderizar si ya tenemos config
      if (currentConfig) {
        renderWidget(currentConfig);
      }
    });
    
    // Configurar listeners para selector de país
    setupCountrySelector();
    setupMutationObserver();
    
    // Fetch config
    fetchConfig(lsData.storeId, lsData.productId, function(err, config) {
      if (err) {
      ...
      if (!config || !config.enabled) {
        log('Widget not enabled for this store');
        return;
      }
      
      // Si no tenemos geolocalización aún, aplicar fallback
      if (geoLocation.source === 'pending') {
        applyFallbackGeo();
      }
  ...
  if (document.readyState === 'loading') {
    ...
    // Pequeño delay para asegurar que LS esté disponible
    setTimeout(init, 100);
  }
  ...
  window.TiendaSync = {
    ...
    clearCache: function() {
      ...
      keys.forEach(function(key) {
        if (key.indexOf(CONFIG.CACHE_KEY) === 0 || key.indexOf(CONFIG.GEO_CACHE_KEY) === 0) {
          localStorage.removeItem(key);
        }
      });
      log('Cache cleared');
    ...
    setGeoLocation: function(geo) {
      // Permite setear manualmente la geolocalización para testing
      geoLocation = {
